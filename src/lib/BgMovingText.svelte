...
<div id="bg-moving-text">
  <div class="overflow-text" style="--rows: {rows};">
...
<style>
  ...
  .bg__animated-row {
    min-width: calc(var(--h) * 8 / 3);
    overflow: hidden;
    display: grid;
    grid-template-columns: 1fr 1fr;
  }
  .bg__animated-row svg {
    height: calc(var(--h) / var(--rows, 1));
  ...
  @media (prefers-color-scheme: dark) {
    ...
    :global(:root[theme="light"] .overflow-text) {
      opacity: 0.1;
      filter: blur(4px);
    }
  ...
  @media (prefers-color-scheme: light) {
    ...
    :global(:root[theme="dark"] .overflow-text) {
      opacity: 0.1;
      filter: blur(8px);
    }
