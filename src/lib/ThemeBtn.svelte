<script>
  ...
  import { systemColor, theme } from "../store";
  ...
  systemColor.subscribe((systemTheme) => {
    if (!$theme) {
      // when app load
      darkMode = systemTheme === "dark";
      // load preffered theme from browser, if not default o systemTheme
      $theme = systemTheme;
    }
  });
</script>
...
<style>
  ...
  :global(:root[theme="dark"] .toggle__icon) {
